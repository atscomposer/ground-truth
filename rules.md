...
## Rule 11 — Ground Every Risk and Maturity Claim in a Recognized Framework

A narrative's confidence should be checkable, not self-declared. Whenever a document claims a control is "in place," describes posture as "strong" or "mature," or asserts coverage meets a bar (e.g., "MFA is deployed," "we've adopted zero trust"), check the claim against `reference/identity-security-framework.md` — built on NIST SP 800-63 assurance levels (IAL/AAL/FAL) and the CISA Zero Trust Maturity Model identity pillar.

Two things to check:
1. **Is the claim specific enough to locate on the framework at all?** "MFA is deployed" doesn't say whether that's SMS OTP (weak AAL2, phishable) or FIDO2/WebAuthn (AAL3, phishing-resistant) — and that difference is exactly the kind of thing a security-literate reader (auditor, sponsor, peer security leader) will ask about. If the claim is too vague to place on a recognized scale, flag it as an **ungrounded claim**.
2. **Does the evidence in the document match the maturity level being claimed?** If the narrative says "advanced" or "optimal" zero trust identity maturity but describes capabilities that map to "initial" per CISA's model (e.g., no continuous validation, no automated lifecycle), flag the mismatch directly — name which stage the described evidence actually supports.

This rule does not require you to become an architecture reviewer (that stays out of scope per `identity.md`) — you're checking whether the narrative's own claims are benchmarked against something external and recognized, not auditing the underlying implementation. As always: name the mismatch and the framework tier, don't supply the fix.
